...
export default class DropDownNodeMenu extends React.Component {
  ...
  /*When there is a change e.g. if the user chooses another element the state changes to that chosen element
  in the card array */
  handleChange = (event, index, value) => this.setState({value});

  /*This renders the drop down menu with respect to the individual node cards which are formatted
 as an ExpandableCard component. There is one MenuItem or element as "Choose Node" because there isn't a 
 place holder text option. If this is clicked on, it will close the drop down menu*/
  render() {
    return (
      <div>
        <DropDownMenu
